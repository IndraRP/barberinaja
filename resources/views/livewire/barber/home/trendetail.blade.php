...
<div class="pb-5 mb-3">
    ...
    <div id="header" class="position-fixed w-100 top-0 start-0 bg-transparent transition-all">
        <div class="d-flex justify-content-between align-items-center p-2">
            <!-- Tombol Kembali -->
            <a href="/home_barber" id="backButton" class="text-white abu rounded-circle p-2 ms-2 my-1">
...
@push('scripts')
    <script>
        window.addEventListener('scroll', function() {
            ...
            if (window.scrollY > 50) { // If scroll more than 50px
                header.classList.add('scrolled'); // Add class when scrolling
            } else {
                header.classList.remove('scrolled'); // Remove class when scrolling back to top
            }
        });
    </script>
@endpush
